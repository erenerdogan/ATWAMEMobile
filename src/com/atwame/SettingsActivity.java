...
public class SettingsActivity extends SherlockActivity implements
	...
	private CheckBox genelCB, duyuruCB, haberCB, eglenceCB, kisiselCB,
			reklamCB;
	...
	private void settingCategory() {
		CategoryDao categoryDao = ModelFactory
				.getInstance(SettingsActivity.this).getDaoSession()
				.getCategoryDao();
	...
	private void displayContent() {
		...
		UserDao userDao = ModelFactory.getInstance(SettingsActivity.this)
		...
		for (Category category : array) {
			if (category.getId() == CategoryEnum.GENEL.getValue())
				genelCB.setChecked(category.getMember());
			else if (category.getId() == CategoryEnum.DUYURU.getValue())
				duyuruCB.setChecked(category.getMember());
			else if (category.getId() == CategoryEnum.EGLENCE.getValue())
				eglenceCB.setChecked(category.getMember());
			else if (category.getId() == CategoryEnum.HABER.getValue())
				haberCB.setChecked(category.getMember());
			else if (category.getId() == CategoryEnum.KISISEL.getValue())
				kisiselCB.setChecked(category.getMember());
			else if (category.getId() == CategoryEnum.REKLAM.getValue())
				reklamCB.setChecked(category.getMember());
		}
	...
	private class SaveListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			for (Category category : array) {
				if (category.getId() == CategoryEnum.GENEL.getValue())
					category.setMember(genelCB.isChecked());
				else if (category.getId() == CategoryEnum.DUYURU.getValue())
					category.setMember(duyuruCB.isChecked());
				else if (category.getId() == CategoryEnum.EGLENCE.getValue())
					category.setMember(eglenceCB.isChecked());
				else if (category.getId() == CategoryEnum.HABER.getValue())
					category.setMember(haberCB.isChecked());
				else if (category.getId() == CategoryEnum.KISISEL.getValue())
					category.setMember(kisiselCB.isChecked());
				else if (category.getId() == CategoryEnum.REKLAM.getValue())
					category.setMember(reklamCB.isChecked());
	...
	public void gotoWallMe() {
		// TODO Auto-generated method stub
		Intent intent = new Intent(SettingsActivity.this, WallMeActivity.class);
		startActivity(intent);
		overridePendingTransition(R.anim.push_left_in, R.anim.push_left_out);
	}
